...
public class JSONDataParser {
     ...
     */
    private final Map<Integer, Area> areas = new java.util.LinkedHashMap<Integer, Area>();

    /**
     *  lookup for {@link Combatant}s (maps the-west player IDs to model objects)
     */
    private final Map<Integer, Combatant> westPlayers = new org.apache.commons.collections4.map.HashedMap<Integer, Combatant>();

    /**
     *  where the players placed the combatants (maybe multiple on same field)
     */
    private final Map<Combatant, CombatantState> preBattleCombatantStates = new org.apache.commons.collections4.map.HashedMap<Combatant, CombatantState>();

    /**
     *  where the player actual stands at the beginning
     */
    private final Map<Combatant, CombatantState> initialCombatantStates = new java.util.LinkedHashMap<Combatant, CombatantState>();
    ...
    public Fortbattle getBattle() throws JSONException {
        ...
        return new Fortbattle(getFortname(), combatants, battlefield, actions, new AllStatistics(combatantStats, battleStats, roundStatistics), getMapImage());
    }
     ...
     *  @throws JSONException
     */
    private void parsePlayers(final Set<Combatant> players, final CombatantSide side, final JSONArray list) throws JSONException {
        final String posKey;
     ...
     *  @throws JSONException
     */
    private void createPreBattleState(final JSONObject playerData, final CombatantState firstRoundState, final Combatant player) throws JSONException {
     ...
     *  @throws JSONException
     */
    private Combatant createCombatant(final CombatantSide side, final JSONObject playerData, final CombatantState firstRoundState) throws JSONException {
        ...
        final Combatant player = new SimpleCombatant(side, firstRoundState, maxLP, playerName, charClass, charLevel, weapon, city);
        return player;
    }
                ...
                try {
                    ...
                    try {
                        for (int i = 0; i < tiles.length(); i++) {
                            ...
                            if (LOG.isLoggable(Level.FINE)) {
                                LOG.fine("taking tile at " + x * tileSizeX + "/" + y * tileSizeY + " with size " + width * tileSizeX + "x" + height * tileSizeY);
                            }
                            g.drawImage(texture, mapX * tileSizeX, mapY * tileSizeY, (mapX + width) * tileSizeX, (mapY + height) * tileSizeY, x * tileSizeX, y * tileSizeY, (x + width) * tileSizeX, (y + height) * tileSizeY, null);
                        }
                    } catch (final JSONException e) {
                        e.printStackTrace();
                    } finally {
     ...
     *  @throws JSONException
     */
    private Set<Area> parseCells(final JSONArray cellData, final List<Sector> sectors, final MapCoordinates mapCoordinates) throws JSONException {
    ...
    protected List<Sector> parseSectors(final JSONArray sectorData) throws JSONException {
        ...
        for (int i = 0; i < sectorData.length(); i++) {
            ...
            final Sector newSector = new SimpleSector(height, defenderSpawn, attackerSpawn, attackerBonus, defenderBonus, isFlag, classBonus, bonusClass);
    ...
    protected Battleplan getPlan(final BattleStatistics stats) throws JSONException {
        ...
        if (turns != null) {
            /* states = */
            saveRound(currentRound, rounds, turns, states);
        }
     ...
     *  @return
     */
    private Map<Combatant, CombatantState> saveRound(final int roundNo, final List<Round> rounds, final List<CombatantTurn> turns, final Map<Combatant, CombatantState> states) {
     ...
     *  @param actions
     */
    private void createTargetActions(final Map<Combatant, CombatantState> combatantStates, final List<CombatantTurn> actions) {
